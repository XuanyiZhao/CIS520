...
for p = 1:length(initparams)
    init_b1{p} = load(['../Synthetic-Dataset/InitParams/relu/',num2str(initparams(p)),'/b1.txt']);
    init_b2{p} = load(['../Synthetic-Dataset/InitParams/relu/',num2str(initparams(p)),'/b2.txt']);
    init_w1{p} = load(['../Synthetic-Dataset/InitParams/relu/',num2str(initparams(p)),'/W1.txt']);
    init_w2{p} = load(['../Synthetic-Dataset/InitParams/relu/',num2str(initparams(p)),'/W2.txt']);
end
...
for j = 1:6
    ...
    for k = 1:5
        ...
        [w1, w2, b1, b2] = NN_ReLU_model(temp_train_x, temp_train_y, ...
            init_w1{j}, init_w2{j}, init_b1{j}, init_b2{j}, 8000, 0.01);
